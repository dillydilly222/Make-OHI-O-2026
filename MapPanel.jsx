...
import { VEHICLE_REFRESH_MS, WEATHER_REFRESH_MS } from '../constants';
import { fetchRoute, fetchVehicles, fetchWeather, weatherCodeToText } from '../utils';
...
export default function MapPanel({ routes }) {
  ...
  const [weatherStatus, setWeatherStatus] = useState('Weather idle.');
  ...
  const loadWeather = useCallback(async () => {
    ...
    try {
      ...
      setWeather(current);
      setWeatherStatus(`Updated ${new Date().toLocaleTimeString()}`);
    } catch (error) {
      setWeatherStatus(`Weather fetch failed: ${error.message}`);
    }
  }, []);
    ...
    <section className="panel map-panel">
      ...
      <div className="weather-card">
        <div className="panel-title-wrap">
          <h2>Columbus Weather</h2>
          <button className="btn" onClick={loadWeather}>Refresh Weather</button>
        </div>
        <p className={`sampling-note ${weatherStatus.includes('failed') ? 'is-warn' : 'is-good'}`}>{weatherStatus}</p>
        <div className="metrics-row weather-row">
          ...
          <article className="metric">
            ...
            <p className="metric-value">{weather ? `${Math.round(weather.windSpeed)} mph` : '--'}</p>
          </article>
          <article className="metric">
            <p className="metric-label">Humidity</p>
            <p className="metric-value">{weather ? `${Math.round(weather.humidity)}%` : '--'}</p>
          </article>
        </div>
      </div>
